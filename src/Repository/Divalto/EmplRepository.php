...
class EmplRepository extends ServiceEntityRepository
{
    ...
    public function getBadPlaceProduct(): array
    {
		...
		RTRIM(LTRIM(v.OP)) AS opVtl, RTRIM(LTRIM(v.PICOD)) AS typePiece, RTRIM(LTRIM(s.DEPOT)) AS depotStock, SUM(s.QTETJSENSTOCK) AS stock, RTRIM(LTRIM(s.EMPLACEMENT)) AS emplStock
        FROM(
        ...
        LEFT JOIN MVTL_STOCK_V s ON dos = s.DOSSIER AND ref = s.REFERENCE AND sref1 = s.SREFERENCE1 AND sref2 = s.SREFERENCE2
		INNER JOIN ART a ON a.DOS = dos AND a.REF = ref
		WHERE v.PICOD IN (2,3) AND v.TICOD NOT IN('I') AND (v.LIEU <> empl OR empl <> s.EMPLACEMENT)
        GROUP BY dos, ref, sref1, sref2, a.DES, depot, empl, v.TIERS, v.DEPO, v.LIEU, v.VTLNO, v.PINO, v.OP, v.PICOD, s.DEPOT, s.EMPLACEMENT
        ORDER BY ref
        ";
