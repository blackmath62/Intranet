...
class ComptaAnalytiqueController extends AbstractController
{
    ...
    public function getRapport($annee, $mois)
    {
        ...
        for ($lig = 0; $lig < count($exportVentes); $lig++) {
            ...
            //}
            if ($achat) {$pa = $achat['pa'];}
            $ventes[$lig]['Cma'] = $pa;
            $crt = 0;
            $cmpt = 0;
            $cmat = 0;
            if ($exportVentes[$lig]['CoutRevient'] != 0 && $exportVentes[$lig]['qteVtl'] != 0) {$crt = abs($exportVentes[$lig]['qteVtl']) * $exportVentes[$lig]['CoutRevient'];}
            $ventes[$lig]['TotalCoutRevient'] = $crt;
            if ($exportVentes[$lig]['CoutMoyenPondere'] != 0 && $exportVentes[$lig]['qteVtl'] != 0) {$cmpt = abs($exportVentes[$lig]['qteVtl']) * $exportVentes[$lig]['CoutMoyenPondere'];}
