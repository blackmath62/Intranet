...
class StatesController extends AbstractController
{
    ...
    public function __construct(LoggerInterface $logger, ProgressManager $progressManager, MailListRepository $repoMail, MailerInterface $mailer)
    {
    ...
    #[Route("/Lhermitte/states/excel/detaille", name: "app_states_lhermitte_excel_detaille")]
    #[Route("/Roby/states/excel/detaille", name: "app_states_roby_excel_detaille")]

    public function statesExcelDetaille(StatesByTiersRepository $repo, Request $request): Response
    {
        // initialisation de mes variables
        if ($request->attributes->get('_route') == 'app_states_roby_excel_detaille') {
            //$form = $this->createForm(YearMetierLFType::class);
            //$metier = 'RB';
        } elseif ($request->attributes->get('_route') == 'app_states_lhermitte_excel_detaille') {
            $form    = $this->createForm(YearMetierLFType::class);
            $dossier = 1;
        }
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $data = $repo->getStatesExcelDetaille($form->getData()['Metiers'], (int) $form->getData()['year'], $dossier);
            $this->get_states_excel_metier_detaille($form->getData()['Metiers'], (int) $form->getData()['year'], $data);
            return $this->redirectToRoute("app_states_lhermitte_excel_detaille");
        }

        return $this->render('states_lhermitte/excel_Detaille.html.twig', [
            'title' => 'States Excel Détaillé',
    ...
    #[Route("/Lhermitte/detaille/excel/{metier}/{year}/{dos}", name: "app_states_excel_metier_Lh_tres_detaille")]
    #[Route("/Roby/detaille/excel/{metier}/{year}/{dos}", name: "app_states_excel_metier_Rb_tres_detaille")]

    public function get_states_excel_metier_detaille($metier, $year, $data)
    {
        ini_set('memory_limit', '4G');
        ini_set('max_execution_time', 0);
        $tempsExec = "";
        ...
        $sheet->getCell('A5')->setValue('Commercial');
        $sheet->getCell('B5')->setValue('Famille_client');
        $sheet->getCell('C5')->setValue('Client');
        $sheet->getCell('D5')->setValue('Nom');
        $sheet->getCell('E5')->setValue('Pays');
        $sheet->getCell('F5')->setValue('Famille_article');
        $sheet->getCell('G5')->setValue('Ref');
        ...
        $sheet->getCell('K5')->setValue('Uv');
        $sheet->getCell('L5')->setValue('Mois');
        $sheet->getCell('M5')->setValue('Op');
        $sheet->getCell('N5')->setValue('Qte Cmd');
        $sheet->getCell('O5')->setValue('Pu Cmd');
        $sheet->getCell('P5')->setValue('Montant Cmd Active');
        $sheet->getCell('Q5')->setValue('Qte Bl');
        $sheet->getCell('R5')->setValue('Pu Bl');
        $sheet->getCell('S5')->setValue('Montant Bl Active');
        $sheet->getCell('T5')->setValue('Qte ' . $year);
        $sheet->getCell('U5')->setValue('Pu ' . $year);
        $sheet->getCell('V5')->setValue('Montant ' . $year);
        $sheet->getCell('W5')->setValue('Delta ' . $year . " - " . $year - 1);
        $sheet->getCell('X5')->setValue('Qte ' . $year - 1);
        $sheet->getCell('Y5')->setValue('Pu ' . $year - 1);
        $sheet->getCell('Z5')->setValue('Montant ' . $year - 1);
        $sheet->getCell('AA5')->setValue('Delta ' . $year - 1 . " - " . $year - 2);
        $sheet->getCell('AB5')->setValue('Qte ' . $year - 2);
        $sheet->getCell('AC5')->setValue('Pu ' . $year - 2);
        $sheet->getCell('AD5')->setValue('Montant ' . $year - 2);
        $sheet->getCell('AE5')->setValue('Delta ' . $year - 2 . " - " . $year - 3);
        $sheet->getCell('AF5')->setValue('Qte ' . $year - 3);
        $sheet->getCell('AG5')->setValue('Pu ' . $year - 3);
        $sheet->getCell('AH5')->setValue('Montant ' . $year - 3);

        $d          = new DateTime('NOW');
        $dateTime   = $d->format('d-m-Y');
        $nomFichier = 'States Excel détaillées Métier =>' . $metier . ' - ' . $year - 3 . ' - ' . $year . ' le ' . $dateTime;
        // Titre de la feuille
        $sheet->getCell('A1')->setValue($nomFichier);
        $sheet->getCell('A1')->getStyle()->getFont()->setSize(20);
        $sheet->getCell('A1')->getStyle()->getFont()->setUnderline(true);
        ...
        $spreadsheet->getActiveSheet()->getStyle("A5:AH5")->applyFromArray($styleEntete);
        $timeEndEntete       = microtime(true);
        $executionTimeEntete = $timeEndEntete - $timeStartEntete;
        $this->logger->info("Temps d'exécution de la requête Entête : {$executionTimeEntete} secondes");
        ...
        $timeStartFormatCellule = microtime(true);
        // Définir les colonnes concernées par les formats
        $columnsToFormat   = ['O', 'P', 'R', 'S', 'U', 'V', 'Y', 'Z', 'AC', 'AD', 'AG', 'AH'];
        $percentageColumns = ['W', 'AA', 'AE'];
        ...
        $timeStartExportDonnees = microtime(true);
        // Increase row cursor after header write
        $dataMetier = $this->getDataMetierExcelDetaille($data);
        ...
        $timeStartSommeEtFiltre = microtime(true);
                                                                       ...
                                                                       // Sous Total des colonnes pour les colonnes Quantités et montants de chaque années
        $sheet->setCellValue("N4", "=SUBTOTAL(9,N6:N{$dernLign})");    // Quantité Cmd
        $sheet->setCellValue("P4", "=SUBTOTAL(9,P6:P{$dernLign})");    // Montant Cmd
        $sheet->setCellValue("Q4", "=SUBTOTAL(9,Q6:Q{$dernLign})");    // Quantité BL
        $sheet->setCellValue("S4", "=SUBTOTAL(9,S6:S{$dernLign})");    // Montant BL
        $sheet->setCellValue("T4", "=SUBTOTAL(9,T6:T{$dernLign})");    // Quantité year
        $sheet->setCellValue("V4", "=SUBTOTAL(9,V6:V{$dernLign})");    // Montant year
        $sheet->setCellValue("X4", "=SUBTOTAL(9,X6:X{$dernLign})");    // Quantité year -1
        $sheet->setCellValue("Z4", "=SUBTOTAL(9,Z6:Z{$dernLign})");    // Montant year -1
        $sheet->setCellValue("AB4", "=SUBTOTAL(9,AB6:AB{$dernLign})"); // Quantité year - 2
        $sheet->setCellValue("AD4", "=SUBTOTAL(9,AD6:AD{$dernLign})"); // Montant year - 2
        $sheet->setCellValue("AF4", "=SUBTOTAL(9,AF6:AF{$dernLign})"); // Quantité year - 3
        $sheet->setCellValue("AH4", "=SUBTOTAL(9,AH6:AH{$dernLign})"); // Montant year - 3
        $sheet->setCellValue("W4", "=(V4/Z4)-1");                      // Delta year / year - 1
        $sheet->setCellValue("AA4", "=(Z4/AD4)-1");                    // Delta year - 1 / year - 2
        $sheet->setCellValue("AE4", "=(AD4/AH4)-1");                   // Delta year - 2 / year - 3
    ...
    public function getDataMetierExcelDetaille($datas): array
    {
        ini_set('memory_limit', '4G');
        ini_set('max_execution_time', 0);
        $list = [];
        ...
        for ($i = 0; $i < count($datas); $i += $batchSize) {
            $batch = array_slice($datas, $i, $batchSize);
            ...
            foreach ($batch as $data) {
                $batchList[] = [
                    ...
                    $data['Op'],
                    $data['QteSignCmd'],
                    $this->calcul_pu($data['MontantSignCmd'], $data['QteSignCmd']),
                    $data['MontantSignCmd'],
                    $data['QteSignBl'],
                    $this->calcul_pu($data['MontantSignBl'], $data['QteSignBl']),
                    $data['MontantSignBl'],
                    $data['QteSign'],
                    $this->calcul_pu($data['MontantSign'], $data['QteSign']),
                    $data['MontantSign'],
                    $this->calcul_delta($data['MontantSign'], $data['MontantSignN']),
                    $data['QteSignN'],
                    $this->calcul_pu($data['MontantSignN'], $data['QteSignN']),
                    $data['MontantSignN'],
                    $this->calcul_delta($data['MontantSignN'], $data['MontantSignN1']),
                    $data['QteSignN1'],
                    $this->calcul_pu($data['MontantSignN1'], $data['QteSignN1']),
                    $data['MontantSignN1'],
                    $this->calcul_delta($data['MontantSignN1'], $data['MontantSignN2']),
                    $data['QteSignN2'],
                    $this->calcul_pu($data['MontantSignN2'], $data['QteSignN2']),
                    $data['MontantSignN2'],
                ];
            }
