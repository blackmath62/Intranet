...
{
    /**
     * @Route("/activite/metier", name="app_activite_metier")
     */
    public function index(MouvRepository $repo, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $activites = $repo->getActivitesMetier($dd, $df, $metier);
            $total = $repo->getTotalActivitesMetier($dd, $df, $metier);
            $activitesClient = $repo->getActivitesMetierClient($dd, $df, $metier);

            // Données stats camenbert Produits
            $dataProduit = $repo->getActivitesFamilleProduit($dd, $df, $metier);
            ...
            $dataClient = $repo->getActivitesFamilleClient($dd, $df, $metier);
